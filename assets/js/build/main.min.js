!function(global, factory) {
    ...
    function handler(event) {
        ...
        return event = $.event.fix(orgEvent), event.type = "mousewheel", orgEvent.wheelDelta && (delta = orgEvent.wheelDelta / 120), 
        orgEvent.detail && (delta = -orgEvent.detail / 3), deltaY = delta, void 0 !== orgEvent.axis && orgEvent.axis === orgEvent.HORIZONTAL_AXIS && (deltaY = 0, 
        deltaX = -1 * delta), void 0 !== orgEvent.wheelDeltaY && (deltaY = orgEvent.wheelDeltaY / 120), 
        void 0 !== orgEvent.wheelDeltaX && (deltaX = -1 * orgEvent.wheelDeltaX / 120), args.unshift(event, delta, deltaX, deltaY), 
    ...
    $.event.special.mousewheel = {
        setup: function() {
            if (this.addEventListener) for (var i = types.length; i; ) this.addEventListener(types[--i], handler, !1); else this.onmousewheel = handler;
        },
        teardown: function() {
            if (this.removeEventListener) for (var i = types.length; i; ) this.removeEventListener(types[--i], handler, !1); else this.onmousewheel = null;
        }
    }, $.fn.extend({
...
var Main = function($) {
    return $(document).keyup(function(e) {
        27 === e.keyCode && Main._submitClose();
    }), {
        altToggle: function() {
        ...
        _submitClose: function() {
            $(".submit-modal").is(".-active") && $(".submit-modal").removeClass("-active");
        },
        submitToggle: function() {
            $(".submit-toggle").on("click", function(e) {
                e.preventDefault(), $(".submit-modal").toggleClass("-active");
            });
        },
        insertLists: function() {
            $(".insert-list").each(function() {
                $(this).hasClass("-1") && $(this).html($(".master-list.-1").html()), $(this).hasClass("-2") && $(this).html($(".master-list.-2").html());
        ...
        randomizeLists: function() {
            ...
            }, $(".randomize").randomize("li"), $(".randomize").removeClass("randomize");
        },
        scrollAnim: function() {
            function loopScroll($obj, speed, rand, dir, width) {
                var stepTime = 10, b1 = -width, b2 = 0, maxSpeed = 2e4, accel = 20, decel = 15, restSpeed = 30 * rand;
                window.globalWheel > 0 && (speed += accel * rand * dir), window.globalWheel < 0 && (speed -= accel * rand * dir), 
                0 == window.globalWheel && (speed > restSpeed * -dir && (speed -= decel * rand), 
                -restSpeed * -dir > speed && (speed += decel * rand)), speed = speed > maxSpeed ? maxSpeed : speed, 
                speed = -maxSpeed > speed ? -maxSpeed : speed, pos = $obj.position().left, speed > 0 && pos > b2 && $obj.css({
                    left: b1 + (pos - b2) + "px"
                }), 0 > speed && pos < b1 && $obj.css({
                    left: b2 + (pos - b1) + "px"
                }), $obj.animate({
                    left: "+=" + speed / (1e3 / stepTime)
                }, {
                    duration: stepTime,
                    easing: "linear",
                    complete: function() {
            ...
            $(".scroll").each(function() {
                clone1 = $(this).children().clone(), clone2 = $(this).children().clone(), $(this).append(clone1), 
                $(this).append(clone2);
            }), window.globalWheel = 0, $("body").mousewheel(function(event, delta) {
                window.globalWheel = delta, clearTimeout($.data(this, "timer")), $.data(this, "timer", setTimeout(function() {
                    window.globalWheel = 0, console.log(window.globalWheel);
                }, 250)), console.log(window.globalWheel), event.preventDefault();
            }), $(".scroll").each(function() {
                dir = $(this).hasClass("-right") ? -1 : 1, rand = .4 + .8 * Math.random(), speed = 30 * rand * -dir, 
                numLi = $(this).find("li").length / 3, width = 500 * numLi, $(this).css({
        ...
        initMain: function() {
            $(document).ready(function() {
                Main.altToggle(), Main.submitToggle(), Main.insertLists(), Main.randomizeLists(), 
                Main.scrollAnim();
            });
        }
    };
